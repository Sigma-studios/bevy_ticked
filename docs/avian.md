...
| `SolverConfig::warm_start_coefficient` | `0.0` | Warm starting seeds the solver with the previous step's impulses; a replayed tick has a different previous step. |
...
Opt-outs, each for a solo game that never rolls back: `keep_warm_starting()`,
`allow_sleeping()`, `positions_from_transforms()`. `physics_added_by_the_game()` when
